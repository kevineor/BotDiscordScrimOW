...
class Discord_scrim:
    ...
    def add_register_player(self, discord_id, is_tank: bool, is_dps: bool, is_heal: bool):
    ...
    def get_nb_players_roles(self):
        ...
        nb_heal = 0
        for player in self.scrim_brain.players_list:
            is_tank, _, _, is_dps, _, _, is_heal, _, _ = self.get_sum_up_player(player.discord_id)
            if is_tank:
                nb_tank += 1
            if is_dps:
                nb_dps += 1
    ...
    def _temp_add_bots(self, nb):
        ...
        for i, name in enumerate(names[:nb]):
            ...
            player = self.add_register_player(
                discord_id, role_bool_matrix[i, 0], role_bool_matrix[i, 1], role_bool_matrix[i, 2]
            )
    ...
    def get_panel_namesanddiscordidrole_team(self, team_id):
        dict_players_id = self.scrim_brain.get_panel_namesanddiscordid_team(team_id)
        if team_id == "bench":
            return dict_players_id
        roles = ["tank1", "dps1", "dps2", "heal1", "heal2"]
        ...
        for role in roles:
            if role not in roles_selected:
                dict_players_id[role] = f"{dict_emoji_role[role[:-1]]} " f"{role[-1]} leaver"
        return dict_players_id
