...
def add_player_to_team(player: Player, team: dict, target_priority: int = None) -> bool:
    """
    player : Player
    team : dict
    ...
    if (team["tank1"] is None) and player.roles.get_tank_is_choosen():
        if player.roles.get_tank_priority() == target_priority:
            team["tank1"] = player
            return True

    if (team["dps1"] is None or team["dps2"] is None) and player.roles.get_dps_is_choosen():
    ...
    if (team["heal1"] is None or team["heal2"] is None) and player.roles.get_heal_is_choosen():
...
def get_score_elo_dict(teams_dict: dict) -> dict:
    ...
    for team_id, team in teams_dict.items():
        if team_id == "bench":
            continue
        score_elo_dict[team_id] = abs(get_team_elo_mean(team_id, teams_dict) - mean) / mean
...
def switch_2_players_custom(teams_dict: dict, potential_player: Player, team_id: int, role: str):
    player = teams_dict[team_id][role]
    teams_dict["bench"].append(player)
    teams_dict[team_id][role] = potential_player


def switch_2_players_custom_2(teams_dict: dict, potential_player: Player, team_id: int, role: str):
    player = teams_dict[team_id][role]
    teams_dict[team_id][role] = potential_player
    return player


def switch_2_players_custom_team(
    teams_dict: dict,
...
def try_switch_bench(
    potential_player: Player, team_id: int, role: str, teams_dict: dict, best_position: dict
):
    ...
    if create_global_score(score_elo_scrim, score_priority_scrim) < create_global_score(
        best_position["score_elo"], best_position["score_priority"]
    ):
...
def bench_rotation(
    ...
    for i in range(nb_iteration_max_bench):
        ...
        best_position = {"score_elo": 1, "score_priority": 1, "team_id": None, "role": None}
        for team_id in range(nb_teams):
            if potential_player.roles.tank["Is_choosen"]:
                best_position = try_switch_bench(
                    potential_player,
                    team_id,
                    role="tank1",
        ...
        if create_global_score(score_elo_scrim, score_priority_scrim) > create_global_score(
            ...
            if best_position["team_id"] is None:
                ...
                old_player = switch_2_players_custom_2(
                    teams_dict, potential_player, best_position["team_id"], best_position["role"]
                )
...
def build_team_main_1(player_list):
    ...
    for k in range(nb_teams):
        teams_dict[k] = {"tank1": None, "dps1": None, "dps2": None, "heal1": None, "heal2": None}
    ...
    for priority in range(2, 4):
        ...
        for player in player_pool:
            i = 0
            ok = True
            while add_player_to_team(player, teams_dict[i], priority) == False:
                i += 1
                if i == nb_teams:
                    ok = False
                    break
