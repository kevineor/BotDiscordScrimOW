...
class Bot(commands.Bot):
    ...
    def __set_elo_options(self):
        self.elo_options = [
            discord.SelectOption(label="1-5", value=6, emoji=self.dict_emojis[6]),
            discord.SelectOption(label="1-5", value=5, emoji=self.dict_emojis[5]),
            discord.SelectOption(label="1-5", value=4, emoji=self.dict_emojis[4]),
            discord.SelectOption(label="1-5", value=3, emoji=self.dict_emojis[3]),
            discord.SelectOption(label="1-5", value=2, emoji=self.dict_emojis[2]),
            discord.SelectOption(label="1-5", value=1, emoji=self.dict_emojis[1]),
            discord.SelectOption(label="1-5", value=0, emoji=self.dict_emojis[0]),
        ]
    ...
    async def send_main_msg(self, ctx, content=None):
        if bot.discrim.can_register():
            ...
            try:
                ...
                await self.main_message.edit(content=content, embed=embed, view=view)
            except:
                self.main_message = await ctx.channel.send(content=content, embed=embed, view=view)
            ...
            try:
                await self.main_message.edit(embed=None)
                await self.main_message.edit(content=content, embeds=embeds, view=view)
            except:
                self.main_message = await ctx.channel.send(
...
async def get_panel_scrim(ctx):
    ...
    if not ctx.author.guild_permissions.administrator:
        await ctx.interaction.response.send_message(
            content="Tu n'as pas les droits", ephemeral=True
        )
        return
    if not CHANNEL_BOT in ctx.channel.name:
        await ctx.interaction.response.send_message(
            content=f"Utilise le channel {CHANNEL_BOT}", ephemeral=True
        )
        return
...
def get_sum_up_selections_embed(discord_id):
    [
        ...
        prority_heal,
        elo_heal,
    ] = bot.discrim.get_sum_up_player(discord_id)
    indices_embed = []
    embed = discord.Embed(title="Inscription terminée", color=discord.Color.random())
    if is_tank:
        ...
        embed.insert_field_at(
            ...
            value=f"{bot.priority_emojis[prority_tank]}" f"{bot.dict_emojis[elo_tank]}➖",
        )
    if is_dps:
        ...
        embed.insert_field_at(
            ...
            value=f"{bot.priority_emojis[prority_dps]}" f"{bot.dict_emojis[elo_dps]}➖",
        )
    if is_heal:
        ...
        embed.insert_field_at(
            ...
            value=f"{bot.priority_emojis[prority_heal]}" f"{bot.dict_emojis[elo_heal]}➖",
        )
    return embed
...
def get_teams_embeds():
    ...
    for i in range(nb_teams):
        if ((i + 1) == nb_teams) and is_bench_team:
            embed = discord.Embed(
                title="~  🪑  ~ LE BANC (cheh) ~  🪑  ~", color=discord.Color.random()
            )
            ...
            for info in bot.discrim.get_team_players_info(i):
                ...
                value = " ➖ "
                if is_tank:
                    value += f'{bot.role_emojis["tank"]}' f"{bot.dict_emojis[elo_tank]}➖"
                if is_dps:
                    value += f'{bot.role_emojis["dps"]}' f"{bot.dict_emojis[elo_dps]}➖"
                if is_heal:
                    value += f'{bot.role_emojis["heal"]}' f"{bot.dict_emojis[elo_heal]}➖"
                embed.set_field_at(
                    index=dict_order_role[role_player] + rank_role_player,
                    name=bot.role_emojis[role_player] + "  " + name_player,
                    value=value[:-1] + " ➖",
                    inline=False,
                )
        embeds.append(embed)
...
class View_panel_begin_registration(discord.ui.View):
    ...
    @discord.ui.button(
        label="Démarrer un Scrim", style=discord.ButtonStyle.green, custom_id="start_scrim"
    )
...
class View_panel_end_registration(discord.ui.View):
    ...
    @discord.ui.button(
        label="Création des équipes", style=discord.ButtonStyle.green, custom_id="create_teams"
    )
    ...
    @discord.ui.button(
        label="Liste joueurs", style=discord.ButtonStyle.blurple, custom_id="get_list_players"
    )
    async def btn_get_list_player_callback(self, button, interaction):
        ...
        for player in bot.discrim.get_list_player():
            ...
            [
                ...
                priority_heal,
                elo_heal,
            ] = bot.discrim.get_sum_up_player2(player)
    ...
    @discord.ui.button(
        label="Annuler le Scrim", style=discord.ButtonStyle.red, custom_id="stop_scrim"
    )
...
class View_register_button(discord.ui.View):
    ...
    @discord.ui.button(
        label="S'inscire", style=discord.ButtonStyle.green, custom_id="initialisation_register"
    )
    async def btn_register_callback(self, button, interaction):
        if bot.discrim.can_register():
            if bot.discrim.is_registered(interaction.user):
                await interaction.response.send_message(
                    content="Vous êtes déjà inscrit !", ephemeral=True
                )
            else:
            ...
            await interaction.response.send_message(
                content="Les inscriptions sont terminées !", ephemeral=True
            )
    ...
    async def btn_unregister_callback(self, button, interaction):
        if bot.discrim.can_register():
            if bot.discrim.is_registered(interaction.user):
                ...
                await bot.send_main_msg(ctx)
                await interaction.response.send_message(
                    content="Vous avez quitté le SCRM, coward ..", ephemeral=True
                )
            else:
                await interaction.response.send_message(
                    content="Vous n'êtes même pas inscrit", ephemeral=True
                )
        else:
            await interaction.response.send_message(
                content="Les inscriptions sont terminées !", ephemeral=True
...
class View_register_request(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(label="Tank", value="tank", emoji=bot.role_emojis["tank"]),
            discord.SelectOption(label="DPS", value="dps", emoji=bot.role_emojis["dps"]),
            discord.SelectOption(label="Heal", value="heal", emoji=bot.role_emojis["heal"]),
        ],
    )
    ...
    @discord.ui.button(custom_id="suivant", label="Suivant", style=discord.ButtonStyle.green)
...
def get_view_tank_request():
    class View_tank_request(discord.ui.View):
        ...
        @discord.ui.select(
            custom_id=f"priority_selection_tank",
            min_values=1,
            max_values=1,
            placeholder=f"Choisis ta priorité pour le tank",
            options=[
                discord.SelectOption(
                    label=f"{bot.priority_emojis[1]} J'ai " f"vraiment envie de jouer tank",
                    value=1,
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[2]} Je peux " f"jouer tank", value=2
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[3]} Je joue " f"tank seulement si nécessaire",
                    value=3,
                ),
            ],
        )
        async def select_temp1_callback(self, select, interaction):
            bot.discrim.set_priority_register(interaction.user, "tank", int(select.values[0]))

        @discord.ui.select(
            custom_id=f"elo_selection_tank",
            min_values=1,
            max_values=1,
        ...
        async def select_temp2_callback(self, select, interaction):
            bot.discrim.set_elo_register(interaction.user, "tank", int(select.values[0]))

        @discord.ui.button(custom_id="retour", label="Retour", style=discord.ButtonStyle.red)
...
def get_view_dps_request():
    class View_dps_request(discord.ui.View):
        ...
        @discord.ui.select(
            custom_id=f"priority_selection_dps",
            min_values=1,
            max_values=1,
            placeholder=f"Choisis ta priorité pour le dps",
            options=[
                discord.SelectOption(
                    label=f"{bot.priority_emojis[1]} J'ai " f"vraiment envie de jouer dps", value=1
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[2]} Je " f"peux jouer dps", value=2
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[3]} Je joue " f"dps seulement si nécessaire",
                    value=3,
                ),
            ],
        )
        async def select_temp3_callback(self, select, interaction):
            bot.discrim.set_priority_register(interaction.user, "dps", int(select.values[0]))

        @discord.ui.select(
            custom_id=f"elo_selection_dps",
            min_values=1,
            max_values=1,
        ...
        async def select_temp4_callback(self, select, interaction):
            bot.discrim.set_elo_register(interaction.user, "dps", int(select.values[0]))
...
def get_view_heal_request():
    class View_heal_request(discord.ui.View):
        ...
        @discord.ui.select(
            custom_id=f"priority_selection_heal",
            min_values=1,
            max_values=1,
            placeholder=f"Choisis ta priorité pour le heal",
            options=[
                discord.SelectOption(
                    label=f"{bot.priority_emojis[1]} J'ai " f"vraiment envie de jouer heal",
                    value=1,
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[2]} Je peux " f"jouer heal", value=2
                ),
                discord.SelectOption(
                    label=f"{bot.priority_emojis[3]} Je joue " f"heal seulement si nécessaire",
                    value=3,
                ),
            ],
        )
        async def select_temp5_callback(self, select, interaction):
            bot.discrim.set_priority_register(interaction.user, "heal", int(select.values[0]))

        @discord.ui.select(
            custom_id=f"elo_selection_heal",
            min_values=1,
            max_values=1,
        ...
        async def select_temp6_callback(self, select, interaction):
            bot.discrim.set_elo_register(interaction.user, "heal", int(select.values[0]))
...
class View_sum_up_request(discord.ui.View):
    ...
    @discord.ui.button(
        custom_id="validate_selection", label="Valider", style=discord.ButtonStyle.green
    )
...
class View_panel_registration_over(discord.ui.View):
    ...
    @discord.ui.button(
        custom_id="removeplayer", label="Remove player", style=discord.ButtonStyle.red
    )
    ...
    @discord.ui.button(
        custom_id="switch2player", label="Switch 2 players", style=discord.ButtonStyle.blurple
    )
    ...
    @discord.ui.button(
        custom_id="redoTeams", label="Recreate teams (auto)", style=discord.ButtonStyle.blurple
    )
    ...
    @discord.ui.button(custom_id="ping", label="Ping @everyone", style=discord.ButtonStyle.green)
    async def button_ping_callback(self, button, interaction):
        ctx = await bot.get_application_context(interaction)
        content = "@everyone"
        await bot.send_main_msg(ctx, content)

    @discord.ui.button(custom_id="stop", label="Stop Scrim", style=discord.ButtonStyle.red)
    async def button_stop_callback(self, button, interaction):
        content = "Toutes les informations des joueurs vont être " "définitivement perdues"
...
def get_view_panel_removeplayer():
    ...
    if team_id is None:
        ...
        for discord_id in dict_players.keys():
            options_player.append(
                discord.SelectOption(label=dict_players[discord_id], value=discord_id)
            )
        ...
        player_selected = bot.discrim.get_panel_info("removeplayer", "discord_id")
    ...
    class View_panel_removeplayer(discord.ui.View):
        ...
        async def select_team_callback(self, select, interaction):
            bot.discrim.set_panel_info("removeplayer", "team", select.values[0])
        ...
        async def select_player_callback(self, select, interaction):
            bot.discrim.set_panel_info("removeplayer", "discord_id", select.values[0])
            view = get_view_panel_removeplayer()
            await interaction.response.edit_message(view=view)
...
def get_view_panel_switch2player():
    ...
    if team1_id is None:
        ...
        dict_players = bot.discrim.get_panel_namesanddiscordidrole_team(team1_id)
        options_player1 = []
        for discord_id in dict_players.keys():
            options_player1.append(
                discord.SelectOption(label=dict_players[discord_id], value=discord_id)
            )
        ...
        player1_selected = bot.discrim.get_panel_info("switch2player", "discord_id1")
    ...
    if team2_id is None:
        ...
        dict_players = bot.discrim.get_panel_namesanddiscordidrole_team(team2_id)
        options_player2 = []
        for discord_id in dict_players.keys():
            options_player2.append(
                discord.SelectOption(label=dict_players[discord_id], value=discord_id)
            )
        ...
        player2_selected = bot.discrim.get_panel_info("switch2player", "discord_id2")
    ...
    class View_panel_switch2player(discord.ui.View):
        ...
        async def select_team1_callback(self, select, interaction):
            bot.discrim.set_panel_info("switch2player", "team1", select.values[0])
        ...
        async def select_player1_callback(self, select, interaction):
            bot.discrim.set_panel_info("switch2player", "discord_id1", select.values[0])
            view = get_view_panel_switch2player()
            await interaction.response.edit_message(view=view)
        ...
        async def select_team2_callback(self, select, interaction):
            bot.discrim.set_panel_info("switch2player", "team2", select.values[0])
            view = get_view_panel_switch2player()
            await interaction.response.edit_message(view=view)
        ...
        async def select_player2_callback(self, select, interaction):
            bot.discrim.set_panel_info("switch2player", "discord_id2", select.values[0])
            view = get_view_panel_switch2player()
            await interaction.response.edit_message(view=view)
